...
export const HomePage: React.FC = () => {
  ...
  const filteredCardOptions = cardOptions.filter((option) =>
    ...
    (option.title !== 'Mensajes Masivos' || user?.roles?.includes('admin')) &&
    (option.title !== 'Carga Masiva' || user?.roles?.includes('superadmin')) &&
    ...
    >
      <div className="layout-content">
        <div className="ultra-wide">
          ...
          {/* Cards grid - perfectly centered with equal heights */}
          <div className="w-full flex justify-center">
            <div className="flex flex-wrap justify-center items-start gap-3 sm:gap-4 md:gap-4 lg:gap-5 xl:gap-6 max-w-6xl">
              {filteredCardOptions.map((option, index) => (
                <div key={index} className="flex-shrink-0 self-start">
                  <CardOptionMenu
                    title={option.title}
                    Icon={option.Icon}
                    route={option.route}
